...
class FullTeachingTestEndToEndVideoSessionTests extends BaseLoggedTest {
    ...
    void oneToOneVideoAudioSessionChrome(String mail, String password, String role) { //124+ 232+ 20 set up +8 lines teardown = 564
        ...
        teacher.getDriver().findElement(By.cssSelector("#md-tab-label-0-1"))
                .click();
        ...
        teacher.getWaiter().until(ExpectedConditions.elementToBeClickable(
                By.xpath("//a[contains(@class, 'usr-btn')]")));
        log.info("{} accepts student intervention", teacher.getClientData());
        teacher.getDriver()
        ...
        teacher.getWaiter().until(ExpectedConditions.elementToBeClickable(
                By.xpath("//a[contains(@class, 'usr-btn')]")));
        log.info("{} canceling student intervention", teacher.getClientData());
        teacher.getDriver()
                .findElement(By.xpath("//a[contains(@class, 'usr-btn')]"))
                .click();
        // Wait until only one video
        teacher.getWaiter().until(ExpectedConditions
