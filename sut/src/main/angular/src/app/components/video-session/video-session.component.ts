...
export class VideoSessionComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  private readonly OVConnections: Connection[] = [];
  ...
  ngOnDestroy() {
    ...
    this.leaveSession();
    // Delete the dark overlay and drag-target (if side menu opened) when the component is destroyed
  ...
  getPhotoByName(userName: string) {
    const user = this.course.attenders.find(u => u.nickName === userName);
    return user.picture;
  }
  ...
  changeVolumeIcon(video) {
    ...
    else if (video.volume === 0) this.volumeMuteIcon = "volume_off";
  ...
  joinSession() {
    ...
    this.OVSession.on('streamDestroyed', (event: StreamEvent) => {
      ...
      } else if (this.bigStream.connection.connectionId === stream.connection.connectionId) {
    ...
    this.OVSession.on('signal:chat', (msg: SignalEvent) => {
      const uData: UserData = this.userData.find(d => d.name === JSON.parse(msg.from.data).name);
    ...
    if (this.authenticationService.isTeacher()) {
      this.OVSession.on('signal:askIntervention', (msg: SignalEvent) => {
        ...
        const targetName = JSON.parse(from.data).name;
        this.userData.forEach((uData) => {
          if (uData.name === targetName) {
            uData.interventionRequired = petition;
          }
        });
      });
    }
  ...
  removeUser() {
    this.videoSessionService.removeUser(this.mySessionId).subscribe({
      next: () => { console.log("User left the session"); },
      error: () => { console.error("Error removing user"); }
    });
  }
